const isMobile = /iPhone|iPad|iPod|Android/i.test(navigator.userAgent);
const event = isMobile ? 'touchmove' : 'scroll';

// Obtiene los elementos div1, div2, div3, div4, div5 y div6
const div1 = document.querySelector('#div1');
const div2 = document.querySelector('#div2');
const div3 = document.querySelector('#div3');
const div4 = document.querySelector('#div4');

const margin = 50;

const div1OffsetTop = div1.offsetTop + margin;
const div2OffsetTop = div2.offsetTop + margin;
const div3OffsetTop = div3.offsetTop + margin;
const div4OffsetTop = div4.offsetTop + margin;

// Detecta cuando el usuario hace scroll y llama a la función onScroll
window.addEventListener('scroll', onScroll);

function onScroll() {
  // Obtiene la posición actual del scroll
  const scrollPosition = window.scrollY;

  // Comprueba si se ha llegado a cada uno de los divs y cambia su contenido interno
  if (scrollPosition >= div1OffsetTop && scrollPosition < div2OffsetTop) {
    div1.innerHTML =  `
    <div class="transitionDiv1">
        <h2 class="Text_tittle_Q">Q1</h2>
        <p class="Text_p">Tour Exclusive Monastery (pendiente video del evento)</p>
        <div class="Div_line"></div>
        <p class="Text_p">Lanzamiento Plataforma de anclaje Nft, identificación de Holders compras y descuentos.</p>
    </div>
  `;
  } else if (scrollPosition >= div2OffsetTop && scrollPosition < div3OffsetTop) {
    div2.innerHTML = `
    <div class="transitionDiv2">
        <h2 class="Text_tittle_Q">Q2</h2>
        <p class="Text_p">Implementación chips NFC en las Gorras ancladas al NFT.</p>
        <div class="Div_line"></div>
        <p class="Text_p">Entrega material de la gorra GYATSO.</p>
        <div class="Div_line"></div>
        <p class="Text_p">2 Prendas exclusivas para los eventos vip. 1er Evento Nft Gyatso.</p>
        <div class="Div_line"></div>
        <p class="Text_p">1er Evento Nft Gyatso.</p>
    </div>`;
  } else if (scrollPosition >= div3OffsetTop && scrollPosition < div4OffsetTop) {
    div3.innerHTML = `
    <div class="transitionDiv3">
        <h2 class="Text_tittle_Q">Q3</h2>
        <p class="Text_p">Lanzamiento Cápsulas Educativas para Holders.</p>
        <div class="Div_line"></div>
        <p class="Text_p">Acceso a colecciones limitadas con entrada anticipada.</p>
        <div class="Div_line"></div>
        <p class="Text_p">Participación De Holders en la creación de una nueva gorra con Monastery x RMA.</p>
    </div>`;
  } else if (scrollPosition >= div4OffsetTop) {
    div4.innerHTML = `
    <div class="transitionDiv4">
        <h2 class="Text_tittle_Q">Q4</h2>
        <p class="Text_p">Se Recibira GYATSO para usar en descentraland.</p>
        <div class="Div_line"></div>
        <p class="Text_p">Bikinis Monastery exclusivo para Holders.</p>
        <div class="Div_line"></div>
        <p class="Text_p">Lanzamiento alianzas Exclusive Monastery NFT.</p>
    </div>`;
  }
}